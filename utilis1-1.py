...
def gen_train_data_normal(filename,winlen,strike,threshold,start,end):
    a = pd.read_csv(filename)
    all_value = a['value'].values
    all_label = a['label'].values
    ...
    all_value = all_value[start*1440:end*1440]
    all_label = all_label[start*1440:end*1440]

    all_value = all_value[0:int(len(all_value) * threshold)]
    all_label = all_label[0:int(len(all_label) * threshold)]
...
def gen_labeled_data1(filename,winlen,strike,threshold,start,end):
    a = pd.read_csv(filename)
    all_value = a['value'].values
    all_label = a['label'].values


    all_value = all_value[start*1440:1440 * end]
    all_label = all_label[start*1440:1440 * end]

    all_value = all_value[int(len(all_value) * threshold):len(all_value)]
    all_label = all_label[int(len(all_label) * threshold):len(all_label)]
...
def gen_labeled_data2(filename,threshold,start,end):
    a = pd.read_csv(filename)
    all_value = a['value'].values[start*1440:end*1440]
    all_label = a['label'].values[start*1440:end*1440]

    array = all_value[int(len(all_value) * threshold):len(all_value)]
    label = all_label[int(len(all_label) * threshold):len(all_label)]
...
def get_recall(true_label, test_label, acceptance_delay):
    ...
    return detected_number*1.0/tatol_number
